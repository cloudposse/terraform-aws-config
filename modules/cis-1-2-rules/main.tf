locals {
  ...
  cis_1_2_global_resource_rules = { for key, rule in local.cis_1_2_tagged_rules : key => rule if var.is_global_resource_region && lookup(rule.tags, local.global_only_tag, false) }
