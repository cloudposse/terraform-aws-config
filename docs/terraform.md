...
| <a name="input_sns_encryption_enabled"></a> [sns\_encryption\_enabled](#input\_sns\_encryption\_enabled) | Whether to enable encryption for the SNS topic. Default `false` | `bool` | `false` | no |
| <a name="input_sns_encryption_key_id"></a> [sns\_encryption\_key\_id](#input\_sns\_encryption\_key\_id) | The ID of an AWS-managed customer master key (CMK) for Amazon SNS or a custom CMK. | `string` | `"alias/aws/sns"` | no |
